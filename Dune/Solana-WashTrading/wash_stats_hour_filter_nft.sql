...
with latest_nft_trades as (
    ...
    from (
        ...
        where account_mint = '{{account_mint}}'
    ) temp
...
select 
    ...
    count(trade_tx_index) - sum(is_wash_trade) as non_wash_num,

    avg(amount_usd) as avg_price,
    avg(amount_usd) filter (where is_wash_trade=1) as avg_wash_price,
    avg(amount_usd) filter (where is_wash_trade=0) as avg_non_wash_price
